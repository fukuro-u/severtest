...
module.exports = function() {
	...
	function(err, httpResponse, body){
		try {
			...
			if (content.length > 100) {
				...
				xsmb.findOne({date:date}, {}, function(err, result) {
					if (!!result) {
						...
						console.log({date:date, gdb:db, g1:g1, g2:g2, g3:g3, g4:g4, g5:g5, g6:g6, g7:g7});
			...
			console.log('content get error' + e);
			customRequest();
		}
	});
	let customRequest = function(){request.get({url: 'https://xosothantai.mobi/xsmb-sxmb-xstd-xshn-kqxsmb-ket-qua-xo-so-mien-bac.html',},
		function(err, httpResponse, body){
			try {
				let content = body;
				if (content.length > 100) {
					const { window } = new jsdom.JSDOM(content);
					list = window.document.querySelector('table.kqmb');
					for( item in list){
						let date = item.querySelector('.tit-mien').textContent;
						date = date.match(/[0-9]{1,2}-[0-9]{1,2}-[0-9]{4}/);
						if (date.length > 0) {
							date = date[0];
							let arrD = date.split('-');
						...
						let number = item.getElementsByClassName('number');
