					...
					if (!validator.isLength(str, {min:1, max:250})) {
						...
						TXChat.findOne({}, 'uid value', {sort:{'_id':-1}}, function(err, post) {
							if (!post || post.uid != client.UID || (post.uid == client.UID && post.value != str)) {
								...
								let content = {taixiu:{chat:{message:{user:client.profile.name, value:str,top:getIndex(client.redT.top,client.profile.name)}}}};
...
var cuoc = function(client, data){
	if (!!data && !!data.bet) {
		if (client.redT.TaiXiu_time < 6 || client.redT.TaiXiu_time > 60) {
			...
			if (bet < 1000){
				client.red({taixiu:{err:'Số tiền phải lớn hơn 1000.!!'}});
					...
					if(client.timeCacheTaiXiu == undefined)
						client.timeCacheTaiXiu = now - 4050
					
					
					if(now - client.timeCacheTaiXiu < 4050)
					return client.red({taixiu:{err: 'Bạn đặt quá nhanh'}});
					client.timeCacheTaiXiu = now
						...
						TXCuocOne.findOne({uid:client.UID, phien:phien}, 'bet select', function(isCuocErr, isCuoc) {
							if (!!isCuoc) {
								...
								if (isCuoc.select !== select) {
									...
									Push.create({
										type:"GameTXBet",
										data:JSON.stringify({name:user.name,uid:client.UID,money:bet,select:select})
									});
								}
							}else{
								// cuoc
...
var get_phien = function(client, data){
	if (!!data && !!data.phien) {
		...
		// var getPhien = TXPhien.fndOne({}).sort({ _id: -1 });
