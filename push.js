...
let TelegramTokenDaiLy = process.env.TELEGRAM_FEED_KEY || '';
let TelegramBotDaiLy   = new Telegram(TelegramTokenDaiLy, {polling: true});
// TelegramBotDaiLy.setWebHook(`${process.env.BOT_WEBHOOK}/bot-${TelegramTokenDaiLy}`);
        ...
        if(msg.hasOwnProperty('contact')){
            ...
            TelegramDaiLyBalance.findOne({'form':msg.from.id}, 'phone', function(err, data){
                if (data) {
                    ...
                    TelegramBotDaiLy.sendMessage(msg.from.id, '*HƯỚNG DẪN* ' + 'Hệ thống báo', opts);
                }else{
                    let opts = {
                        parse_mode: 'markdown',
                        reply_markup: {
                    ...
                    TelegramBotDaiLy.sendMessage(msg.from.id, '*tinh* Vui lòng ấn CHIA SẺ SỐ ĐIỆN THOẠI để _XÁC THỰC_', opts);
                }
            });
        }
    });
